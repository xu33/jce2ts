...
rules: [/^(?:\s+)/,/^(?:\/\/(.*))/,/^(?:[/][*][^*]*[*]+([^*/][^*]*[*]+)*[/])/,/^(?:$)/,/^(?:module\b)/,/^(?:struct\b)/,/^(?:enum\b)/,/^(?:interface\b)/,/^(?:<)/,/^(?:>)/,/^(?:\{)/,/^(?:\})/,/^(?:(require|optional))/,/^(?:(string|int|float|long|double|signed\s+int|unsigned\s+int))/,/^(?:vector\b)/,/^(?:map\b)/,/^(?:;)/,/^(?:,)/,/^(?:::)/,/^(?:=\s*[^=;]+)/,/^(?:\d+)/,/^(?:[a-zA-Z_$][a-zA-Z_$0-9]*)/,/^(?:$)/],
